...
const MINES_COEFFICIENTS = [
  [
    0.99, 1.04, 1.09, 1.14, 1.19, 1.26, 1.33, 1.4, 1.49, 1.59, 1.71, 1.84, 1.99,
    2.17, 2.39, 2.65, 2.98, 3.41, 3.98, 4.78, 5.97, 7.96, 11.94, 23.88,
  ],
  [
...
$("#play_btn").on("click", (e) => {
  ...
  $("#nextWin").html(
    (SETTINGS.amount * coefficients[0]).toLocaleString("en-US", {style: "currency", currency: "USD"})
  );
  ...
  $("#gameStatusBar").show();
  $("#play_btn").attr("disabled", "disabled").find("span").text("Ожидание");
...
$("#takePrizeButton").on("click", (e) => {
  ...
  setTimeout(() => {
    ...
    $(".Toastify__toast-container").html(
      `<div class="Toastify--animate Toastify__bounce-enter--top-center Toastify__toast Toastify__toast--close-on-click Toastify__toast--default Toastify__toast-theme--dark app-popup__toast win"><div class="Toastify__toast-body app-popup__toast-body"role=alert><div><div class=app-popup__toast-body__win><div class=left-side><div class=left-side__label>Ваш выигрыш</div><p class=left-side__lightgreen><span id=winValuePopup>${winAmount.toLocaleString(
        "en-US", {style: "currency", currency: "USD"}
      )}</span></div><div class=right-side id=winCoefPopup>x${winCoefficient}</div></div></div></div></div>`
    );
...
function updateSettings() {
  ...
  $("._balance").html(
    SETTINGS.balance.toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
    })
  );

  $("#betAmount").html(SETTINGS.amount.toLocaleString("en-US", {style: "currency", currency: "USD"}));
